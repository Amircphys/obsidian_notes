...
```python
# Выборка данных по условию
query = select(book).where(book.c.amount < 5)
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())
...
query = select(book).where(book.c.price.between(500, 1000))
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())
...
authors_list = ["John Smith", "Jane Doe", "Alice Brown"]

query = select(books).where(books.c.author.in_(authors_list))
result = conn.execute(query)

query = select(books).where(~books.c.author.in_(authors_list))  # ~ означает NOT
result = conn.execute(query)

# Выборка данных с сортировкой
query = select(book).order_by(
book.c.author.desc(), # asc()
)
...
query = select(book).where(
	and_(
		book.c.amount < 5, book.c.price > 700
	)
)
with engine.connect() as conn:
	result = conn.execute(query)
...
# 3. not
query = select(book).where(
	~(book.c.author == "Булгаков М.А.") & (book.c.price > 450) # ~ означает NOT
)
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())
...
query = select(book).where(
	not_(book.c.price.between(500, 600))
)
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())
...
# 4 Сложные комбинированные условия
query = select(book).where(
	((book.c.price < 600) & (book.c.amount > 2)) |
	((book.c.price > 450) & (book.c.amount < 8))
)
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())
...
```python
# like - c учетом регистра, ilike - без учета регистра

query = select(book).where(book.c.author.ilike("%Ский%"))
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())

# IS NULL / IS NOT NULL
query = select(book).where(book.c.price.is_(None))
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())


query = select(book).where(book.c.price.is_not(None))
with engine.connect() as conn:
	result = conn.execute(query)
pd.DataFrame(result.fetchall())
```