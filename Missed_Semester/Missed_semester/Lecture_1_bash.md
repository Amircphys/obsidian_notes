...
* `Ctrl+b x` &mdash; удалить окно
* `Ctrl+d, exit` &mdash; выход из панели, возвращение на предыдущую панель
* `Ctrl+b %` - вертикальные окна
* `Ctrl+b "` - горизонтальные окна
* `tmux new -s myname` - создать новую сессию с названием myname
* `tmux kill-session -t myname` - удалить новую сессию с названием myname
* `tmux a -t myname` - перейти в сессию myname
